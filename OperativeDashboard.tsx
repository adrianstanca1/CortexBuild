...
const OperativeDashboard: React.FC<OperativeDashboardProps> = ({
  currentUser,
  navigateTo,
  onDeepLink,
}) => {
  ...
  return (
    <div className="space-y-8">
      ...
      {!isLoading && (
        <DailyFocusWidget firstTask={firstTask} metrics={metrics} userName={currentUser.name} />
      )}
          ...
          <div className="space-y-8">
            {firstTask ? (
              <div className="bg-white p-6 rounded-lg shadow-lg border border-gray-100">
                ...
                <div
                  onClick={() =>
                    onDeepLink(firstTask.projectId, 'task-detail', { taskId: firstTask.id })
                  }
                ...
                >
                  ...
                  <p className="text-sm text-red-600 font-semibold mt-1">
                    Due: {new Date(firstTask.dueDate).toLocaleDateString()}
                  </p>
                </div>
              </div>
            ) : (
          ...
          <div className="space-y-8">
            <div className="bg-white p-6 rounded-lg shadow-lg border border-gray-100">
              ...
              <button
                ...
                title={
                  !firstTask
                    ? 'You must have an active task on a project to create a daily log'
                    : dailyLog
                      ? "View or edit today's log"
              ...
              >
                {dailyLog
                  ...
                  : `Start Daily Log for ${firstTask?.projectName || 'Project'}`}
              </button>
            </div>
              ...
              <div className="bg-orange-50 rounded-lg shadow-lg border border-orange-200 p-6">
                ...
                <ul className="space-y-3">
                  {siteInstructions.map((inst) => (
                    <li
                      key={inst.id}
                      className="p-4 bg-white/70 backdrop-blur-sm border-l-4 border-orange-500 rounded-r-md shadow-sm"
                    >
