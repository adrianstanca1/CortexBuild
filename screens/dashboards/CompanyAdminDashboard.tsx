...
const CompanyAdminDashboard: React.FC<CompanyAdminDashboardProps> = (props) => {
  const { currentUser, navigateTo, onDeepLink, onQuickAction, onSuggestAction, selectProject } =
    props;
    ...
    <div className="space-y-8">
      <header className="flex justify-between items-start">
        ...
        <img
          ...
          alt="User Avatar"
          className="w-12 h-12 rounded-full shadow-lg border-2 border-blue-200"
        />
      </header>

      {/* Smart Metrics Widget - ML-Powered */}
      {dashboardData && (
        <SmartMetricsWidget metrics={dashboardData.metrics} trends={dashboardData.trends} />
      )}
