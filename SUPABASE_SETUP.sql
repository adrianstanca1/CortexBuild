-- CortexBuild Supabase Schema
-- Run this in your Supabase SQL Editor: https://zpbuvuxpfemldsknerew.supabase.co
...
-- Create policies for companies (allow all for now)
CREATE POLICY "Allow all operations on companies"
    ON companies FOR ALL
    USING (true)
    WITH CHECK (true);

-- Create policies for users (allow all for now)
CREATE POLICY "Allow all operations on users"
    ON users FOR ALL
    USING (true)
    WITH CHECK (true);

-- Create policies for sessions (allow all for now)
